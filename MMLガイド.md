...
対象は、現在の `SN76489` 互換 PSG ベース実装です。
...
- `p<num>`
  チャンネルごとのパンです
  `0=OFF`, `1=右`, `2=左`, `3=左右` です
- `w0`
  periodic noise
- `w1`
  white noise
- `SN76489` モデルでは、`N` チャンネルの音高は近い離散 noise source へ丸められます
  3段の固定 rate と `tone C` 追従 source のいずれかが選ばれます
...
- `@E1={speed, level1, level2, ...}` のように書きます
- 先頭要素 `speed` はエンベロープの進行速度です
- `speed=1` はほぼ毎ティック進み、`speed=2` は 2 ティックごと、`speed=4` は 4 ティックごとに 1 段進みます
- 2 要素目以降は各ステップの音量値です
- 音量値は `0` が最大、`15` が最小です
...
- `@p1={initial, speed, step, delay}` のように書きます
- `initial` はノート開始時のピッチオフセットです
- `speed` は何 tick ごとに変化するかです
- `step` は 1 回ごとの変化量で、負なら下降します
- `delay` は変化開始まで待つ tick 数です
...
この例では、先頭の `1` が速度、後ろの `0,2,4,...` が音量列です。`0` から `14` へ進むので、時間とともに減衰していくエンベロープになります。
...
現在は、MML 上の `o4` をそのまま自然な中域として扱う方針です。

厳密エミュレーションよりも HTML5 ゲーム向けライブラリとしての素直さを優先しているため、テストやブラウザ確認でも `o4` から扱いやすい音域になるようにしています。
