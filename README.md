...
6. 必要に応じて `Branch Select`、`Renderer`、`Chip Model` を切り替える
...
16. `S1` から `S9` で、単純ゲート、立ち上がり、減衰、一瞬だけ落ちて戻る系の内蔵プリセットエンベロープを手早く試せる。番号の意味は `SN76489` / `AY-3-8910` で共通に保ち、実カーブは各チップ向けに最適化する方針
17. `Export Song JSON` で、現在の MML を選択中のチップモデル付き `mkvdrv-song` v1 JSON として書き出せる
